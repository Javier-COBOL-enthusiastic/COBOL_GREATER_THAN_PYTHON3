...
def buscar_objetivo(mapa, x, y):
    ...
    if(mapa[y][x] == "A"):
        return True
    mapa[y][x] = "v"
    r = buscar_objetivo(mapa, x + 1, y)
    r = r or buscar_objetivo(mapa, x - 1, y)
    r = r or buscar_objetivo(mapa, x, y + 1)
    r = r or buscar_objetivo(mapa, x, y - 1)
...
d = buscar_objetivo(mapa, start[1], start[0])
mostrar_mapa(mapa)
print("Objetivo encontrado!" if d else "Objetivo invalido" )